...
statementFieldecl* sintactico::FieldDecl(Token T[])
{
    ...
    if(CurrentToken->tipo == CORCHETE_I)
    {
        ...
        exprIntConstant * entero = new exprIntConstant(atoi(CurrentToken->lexema.c_str()),Lexer->linea);
        ...
        CurrentToken = Lexer->NexToken();
        return new statementFieldecl(tipo,list_only_id,list_id,list_constante,Lexer->linea);
    }
    else if(CurrentToken->tipo == COMA)
    {
        ...
        CurrentToken = Lexer->NexToken();
        return new statementFieldecl(tipo,list_only_id,list_id,list_constante,Lexer->linea); //lista de variables
    }
    else if(CurrentToken->tipo == IGUAL)
    {
        ...
        if(T[0].tipo == KWBOOL)
            ...
            tipo = statementDeclaration::INT;
        return new statementFieldecl(tipo,T[1].lexema,cons,Lexer->linea); //variable inicializada
...
statementMethodecl* sintactico::MethodDecl(Token T[]){
    ...
    if(CurrentToken->tipo == PARENTESIS_I)
    {
        ...
        return new statementMethodecl(type, methodName, paramTypes, paramNames, block,Lexer->linea);
    }
    else
    {
        Fallo = true;
...
var_decl* sintactico::VarDecl()
{
    ...
    if(CurrentToken->tipo == IDENTIFICADOR)
    {
        ...
        CurrentToken = Lexer->NexToken();
        return new var_decl(ids,Lexer->linea);
    }else
    {
        Fallo = true;
        MensajeError("Se esperaba Identificador",Lexer->linea);
...
statement* sintactico::Statement()
{
    if(CurrentToken->tipo == KWIF)
    {
        ...
        return new statementIf(condition, trueBlock, falseBlock,Lexer->linea);
    ...
    else if(CurrentToken->tipo == KWWHILE)
    {
        ...
        return new statementWhile(condition, block,Lexer->linea);
    }
    else if(CurrentToken->tipo == KWFOR)
    {
        ...
        return new statementFor(inits, condition, increments, block,Lexer->linea);
    }
    else if(CurrentToken->tipo == KWRETURN)
    {
        ...
        CurrentToken = Lexer->NexToken();
        return new statementReturn(returnValue,Lexer->linea);
    ...
    else if(CurrentToken->tipo == KWBREAK)
    {
        ...
        CurrentToken = Lexer->NexToken();
        return new statementBreak(Lexer->linea);
    ...
    else if(CurrentToken->tipo == KWCONTINUE)
    {
        ...
        CurrentToken = Lexer->NexToken();
        return new statementContinue(Lexer->linea);
...
statement* sintactico::PrintRead()
{
    ...
    if(CurrentToken->tipo == KWPRINT)
    {
        ...
        result = new statementPrint(args,Lexer->linea);
    }
    else if(CurrentToken->tipo == KWREAD)
    {
        ...
        if(CurrentToken->tipo == IDENTIFICADOR)
        {
            ...
            if(CurrentToken->tipo == CORCHETE_I)
            {
                ...
                CurrentToken = Lexer->NexToken();
                lvalues->push_back(new exprLvalue(id,expr,Lexer->linea));
                ...
                while(CurrentToken->tipo == COMA)
                {
                    ...
                    if(CurrentToken->tipo == IDENTIFICADOR)
                    {
                        ...
                        {
                            lvalues->push_back(new exprLvalue(id,Lexer->linea));
                        }
                    }
                }
            }
        ...
        result = new statementRead(lvalues,Lexer->linea);
    }
    if(CurrentToken->tipo != PUNTO_COMA)
    {
        Fallo = true;
...
statementAssign* sintactico::Assign(string id)
{
    ...
    {
        lValue = new exprLvalue(id,Lexer->linea);
        CurrentToken = Lexer->NexToken();
        value = A();
        if(Fallo)
            return NULL;
        ...
        if (CurrentToken->tipo == CORCHETE_D) {
            CurrentToken = Lexer->NexToken();
            lValue = new exprLvalue(id, index,Lexer->linea);
            if (CurrentToken->tipo == IGUAL) {
                CurrentToken = Lexer->NexToken();
                value = A();
                if(Fallo)
    ...
    return new statementAssign(lValue, value,Lexer->linea);
}
...
statementMethodCall* sintactico::MethodCallStatement(Token T)
{
    if(CurrentToken->tipo == PARENTESIS_I)
    {
        ...
        CurrentToken = Lexer->NexToken();
        return new statementMethodCall(T.lexema, params,Lexer->linea);
    }
    return NULL;
}
...
Expression* sintactico::MethodCallExpression(Token T)
{
    if(CurrentToken->tipo == PARENTESIS_I)
    {
        ...
        CurrentToken = Lexer->NexToken();
        return new exprMethodCall(T.lexema, params,Lexer->linea);
    }
    return NULL;
}
...
exprArgument* sintactico::Argument()
{
    if(CurrentToken->tipo == CADENA)
    {
        ...
        CurrentToken = Lexer->NexToken();
        return new exprArgument(cadena,Lexer->linea);
    }
    Expression* value = A();
    if(Fallo)
        return NULL;
    return new exprArgument(value,Lexer->linea);
}
...
exprLvalue* sintactico::Lvalue()
{
    if(CurrentToken->tipo == CORCHETE_I)
    {
        ...
        CurrentToken = Lexer->NexToken();
        exprLvalue *value = new exprLvalue(Lexer->linea);
        value->setExpr(expr);
        return value;
    } else if (CurrentToken->tipo == IDENTIFICADOR) {
        exprLvalue* lValue = new exprLvalue(CurrentToken->lexema,Lexer->linea);
        CurrentToken = Lexer->NexToken();
        return lValue;
    }
}
...
Expression* sintactico::Constant()
{
    ...
    if (CurrentToken->tipo == INTCONSTANT) {
        expression = new exprIntConstant(atoi(CurrentToken->lexema.c_str()),Lexer->linea);
    } else if (CurrentToken->tipo == CARACTER) {
        char c = CurrentToken->lexema.at(0);
        expression = new exprIntConstant(c,Lexer->linea);
    } else if (CurrentToken->tipo == BOOLCONSTANT) {
        if (CurrentToken->lexema == "false") {
            expression = new exprBoolConstant(false,Lexer->linea);
        } else {
            expression = new exprBoolConstant(true,Lexer->linea);
        }
    } else {
        Fallo = true;
...
Expression* sintactico::A()
{
    ...
    while(CurrentToken->tipo == OR)
    {
        CurrentToken = Lexer->NexToken();
        Expression *der = B();
        izq = new ExprOr(izq,der,Lexer->linea);
    }
    return izq;
}
...
Expression* sintactico::B(){
    ...
    while(CurrentToken->tipo == AND)
    {
        CurrentToken = Lexer->NexToken();
        Expression *der = C();
        izq = new exprAnd(izq,der,Lexer->linea);
    }
    return izq;
}
...
Expression* sintactico::C()
{
    ...
    while(CurrentToken->tipo == DISTINTO)
    {
        CurrentToken = Lexer->NexToken();
        Expression *der = D();
        izq = new exprDistin(izq,der,Lexer->linea);
    }
    while(CurrentToken->tipo == COMPARACION)
    {
        CurrentToken = Lexer->NexToken();
        Expression *der = D();
        izq = new exprEqual(izq,der,Lexer->linea);
    }
...
Expression* sintactico::D()
{
    ...
    while(CurrentToken->tipo == MENOR)
    {
        CurrentToken = Lexer->NexToken();
        Expression *der = E();
        izq = new exprLessThan(izq,der,Lexer->linea);
    }
    while(CurrentToken->tipo == MENOR_IGUAL)
    {
        CurrentToken = Lexer->NexToken();
        Expression *der = E();
        izq = new exprLessAndEqual(izq,der,Lexer->linea);
    }
    while(CurrentToken->tipo == MAYOR)
    {
        CurrentToken = Lexer->NexToken();
        Expression *der = E();
        izq = new exprGreaterThan(izq,der,Lexer->linea);
    }
    while(CurrentToken->tipo == MAYOR_IGUAL)
    {
        CurrentToken = Lexer->NexToken();
        Expression *der = E();
        izq = new exprGreaterAndEqual(izq,der,Lexer->linea);
    }
    return izq;
}
...
Expression* sintactico::E()
{
    ...
    while(CurrentToken->tipo == CORRER_D)
    {
        CurrentToken = Lexer->NexToken();
        Expression *der = F();
        izq = new exprShiftRight(izq,der,Lexer->linea);
    }
    while(CurrentToken->tipo == CORRER_I)
    {
        CurrentToken = Lexer->NexToken();
        Expression *der = F();
        izq = new exprShiftLeft(izq,der,Lexer->linea);
    }
    while(CurrentToken->tipo == ROT)
    {
        CurrentToken = Lexer->NexToken();
        Expression *der = F();
        izq = new exprRot(izq,der,Lexer->linea);
    }
    return izq;
}
...
Expression* sintactico::F()
{
    ...
    while(CurrentToken->tipo == PORCENTAJE)
    {
        CurrentToken = Lexer->NexToken();
        Expression *der = G();
        izq = new exprMode(izq,der,Lexer->linea);
    }
    return izq;
}
...
Expression* sintactico::G()
{
    ...
    while(CurrentToken->tipo == MAS)
    {
        CurrentToken = Lexer->NexToken();
        Expression *der = H();
        izq = new exprAdd(izq,der,Lexer->linea);
    }
    while(CurrentToken->tipo == GUION)
    {
        CurrentToken = Lexer->NexToken();
        Expression *der = H();
        izq = new exprSub(izq,der,Lexer->linea);
    }
    return izq;
}
...
Expression* sintactico::Gp()
{
    if(CurrentToken->tipo == MAS)
    {
        ...
        Expression* der = Gp();
        if(der != NULL)
            return new exprAdd(izq,der,Lexer->linea);
        return izq;
    }
    else if(CurrentToken->tipo == GUION)
    {
        ...
        Expression* der = Gp();
        if(der != NULL)
            return new exprSub(izq,der,Lexer->linea);
        return izq;
    }
    return NULL;
}
...
Expression* sintactico::H()
{
    ...
    while(CurrentToken->tipo == POR)
    {
        CurrentToken = Lexer->NexToken();
        Expression *der = I();
        izq = new exprMult(izq,der,Lexer->linea);
    }
    while(CurrentToken->tipo == PLECA)
    {
        CurrentToken = Lexer->NexToken();
        Expression *der = I();
        izq = new exprDiv(izq,der,Lexer->linea);
    }
    return izq;
}
...
Expression* sintactico::Hp()
{
    if(CurrentToken->tipo == POR)
    {
        ...
        Expression* der = Hp();
        if(der != NULL)
            return new exprMult(izq,der,Lexer->linea);
        return izq;
    }
    else if(CurrentToken->tipo == PLECA)
    {
        ...
        Expression* der = Hp();
        if(der != NULL)
            return new exprDiv(izq,der,Lexer->linea);
        return izq;
    }
    return NULL;
}
...
Expression* sintactico::I()
{
    ...
    {
        CurrentToken = Lexer->NexToken();
        return new exprDeni(expr,Lexer->linea);
    }
    return expr;
}
